...
export default function useIbcCoreChannelV1() {
  ...
  const QueryChannels = (query: any, options: any, perPage: number) => {
    ...
    return useInfiniteQuery([key], ({ pageParam = 1 }: { pageParam?: number }) => {
      ...
      getNextPageParam: (lastPage, allPages) => {
        const total = typeof lastPage.pagination?.total === 'string' ? parseInt(lastPage.pagination.total) : 0;
        const pageParam = lastPage.pageParam || 0;
        if (total > pageParam * perPage) {
          return pageParam + 1;
        } else {
          return undefined;
        }
      ...
      getPreviousPageParam: (firstPage, allPages) => {
        const pageParam = firstPage.pageParam || 0;
        if (pageParam === 1) {
          return undefined;
        } else {
          return pageParam - 1;
  ...
  return {
    QueryChannel, QueryChannels, QueryConnectionChannels, QueryChannelClientState, QueryChannelConsensusState, QueryPacketCommitment, QueryPacketCommitments, QueryPacketReceipt, QueryPacketAcknowledgement, QueryPacketAcknowledgements, QueryUnreceivedPackets, QueryUnreceivedAcks, QueryNextSequenceReceive,
  }
}