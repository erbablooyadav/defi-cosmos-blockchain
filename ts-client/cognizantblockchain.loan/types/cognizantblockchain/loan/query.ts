...
export class QueryClientImpl implements Query {
  ...
  Params(request: QueryParamsRequest): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request("cognizantblockchain.loan.Query", "Params", data);
    return promise.then((data) => QueryParamsResponse.decode(new _m0.Reader(data)));
  }

  Loan(request: QueryGetLoanRequest): Promise<QueryGetLoanResponse> {
    const data = QueryGetLoanRequest.encode(request).finish();
    const promise = this.rpc.request("cognizantblockchain.loan.Query", "Loan", data);
  ...
  LoanAll(request: QueryAllLoanRequest): Promise<QueryAllLoanResponse> {
    ...
    const promise = this.rpc.request("cognizantblockchain.loan.Query", "LoanAll", data);
